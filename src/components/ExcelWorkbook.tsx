  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <header className="shrink-0 border-b border-border bg-card/80 backdrop-blur-md sticky top-0 z-50">
        <div className="flex items-center h-11 px-3 gap-3">
          ...
          <div className="ml-auto flex items-center gap-1">
            <div className="hidden sm:flex items-center gap-0.5 text-[10px] text-muted-foreground">
              <span className="px-2 py-0.5 rounded hover:bg-secondary/40 cursor-default">File</span>
              <span className="px-2 py-0.5 rounded hover:bg-secondary/40 cursor-default">Home</span>
              <span className="px-2 py-0.5 rounded hover:bg-secondary/40 cursor-default">Insert</span>
              <span className="px-2 py-0.5 rounded hover:bg-secondary/40 cursor-default">Data</span>
              <span className="px-2 py-0.5 rounded hover:bg-secondary/40 cursor-default">View</span>
            </div>
            <ThemeToggle />
            <button className="sm:hidden p-1.5" onClick={() => setMobileMenu(!mobileMenu)}>
        ...
        {/* Mobile nav */}
        {mobileMenu && (
          <div className="sm:hidden border-t border-border bg-card p-2 space-y-1">
            {tabs.map((t) => (
              <button
                key={t.id}
      ...
      <div className="shrink-0 border-t border-border bg-card/60 backdrop-blur-sm sticky bottom-0 z-40">
        <div className="flex items-center overflow-x-auto">
          <div className="w-8 md:w-10 shrink-0 flex items-center justify-center border-r border-border text-muted-foreground text-xs">
            ◀▶
          </div>
          {tabs.map((t) => (
            <button
              ...
              className={`px-4 py-2.5 text-xs font-heading font-medium whitespace-nowrap border-r border-border transition-all duration-200 ${
                activeTab === t.id
